...
#if defined(MY_PARAM) || defined(PH_PARAM) //|| defined(KR_PARAM) /*|| defined(MYE_PARAM)*/ //|| defined ( VN_PARAM ) || defined(TH_PARAM) || defined ( HK_PARAM ) || defined(TW_PARAM) // Nprotect ����
...
HRESULT CBasicWnd::CreateObjects()
{
	...
	hr = NLOADINGTHREAD::StartThreadLOAD ( &m_pd3dDevice, m_hWnd, ((CBasicApp*)AfxGetApp())->m_szAppPath, CString("loading_000.dds"),"EnBabyRanYong", true );
...
{	
    ...
    *pEnd = _T('\0');
    ...
    nRet = _AhnHS_Initialize(szFullFileName, AhnHS_Callback, 
		...
		AHNHS_CHKOPT_STANDALONE|AHNHS_CHKOPT_LOADLIBRARY|
        ...
        if (nRet != HS_ERR_OK)
    {
        ...
        MessageBox(szMsg, "UG-RanOnline.com", MB_OK);
        return FALSE;
    }
